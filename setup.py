...
def get_llvm_package_info():
    ...
    if llvm_dir:
        # Return a Package object with the LLVM_DIR path
        print(f"Using LLVM from {llvm_dir}")
        return Package("llvm", "llvm-from-env", "", "LLVM_INCLUDE_DIRS", "LLVM_LIBRARY_DIR", "LLVM_SYSPATH")
...
def get_packages():
    ...
    for backend in backends:
        base_packages.append(f"triton.backends.{backend.name}")

        if backend.language_dir:
            # Install the contents of each backend's `language` directory into
            # `triton.language.extra`.
            for x in os.listdir(backend.language_dir):
                base_packages.append(f"triton.language.extra.{x}")

        if backend.tools_dir:
            # Install the contents of each backend's `tools` directory into
            # `triton.tools.extra`.
            for x in os.listdir(backend.tools_dir):
                base_packages.append(f"triton.tools.extra.{x}")
...
setup(
    ...
    url="https://github.com/chenxingqiang/triton-metal/",
    project_urls={
        "Documentation": "https://chenxingqiang.github.io/triton-metal/",
        "Source": "https://github.com/chenxingqiang/triton-metal/",
        "Tracker": "https://github.com/chenxingqiang/triton-metal/issues",
    },
